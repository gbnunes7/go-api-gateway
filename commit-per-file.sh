...
infer_type() {
  ...
  case "$file" in
    ...
    *_test.go|*_test.ts|test/*|tests/*) echo "test" ;;
    go.mod|go.sum|.gitignore|.env*|Makefile|Dockerfile*|*.yml|*.yaml|*.sh|compose*.yml) echo "chore" ;;
    internal/*|cmd/*|pkg/*|*.go) echo "feat" ;;
    *) echo "chore" ;;
  esac
}

# Lista cada arquivo: -uall lista todos os arquivos dentro de dirs untracked
# while read para compatibilidade com Bash 3 (macOS)
FILES=()
while IFS= read -r line; do FILES+=("$line"); done < <(git status --short -uall | awk '{ if (NF >= 2) print $NF }')
